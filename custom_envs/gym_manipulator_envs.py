    ...
    def step(self, a):
        ...
        electricity_cost = (
            ...
            -0.10 * (np.abs(a[0] * self.robot.theta_dot) + \
                     np.abs(a[1] * self.robot.gamma_dot))
            # stall torque require some energy
            - 0.01 * (np.abs(a[0]) + np.abs(a[1]))
        )
        stuck_joint_cost = - \
            0.1 if np.abs(np.abs(self.robot.gamma) - 1) < 0.01 else 0.0
        self.rewards = [float(self.potential - potential_old),
                        float(electricity_cost), float(stuck_joint_cost)]
...
class StrikerBulletEnv(MJCFBaseBulletEnv):
    ...
    def _step(self, a):
        ...
        dist_object_finger = self.robot.object.pose(
        ).xyz() - self.robot.fingertip.pose().xyz()
